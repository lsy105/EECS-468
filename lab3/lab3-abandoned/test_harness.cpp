...
int main(int argc, char* argv[])
{
    ...
    H.bins = kernel_bins;
    H.width = HISTO_WIDTH;
    H.height = HISTO_HEIGHT;
    Hist Hd = AllocateDeviceHist(H);
    CopyToDeviceHist(Hd, H);

    uint32_t *device_input = AllocateDeviceInput(((INPUT_WIDTH + 128) & 0xFFFFFF80)*INPUT_HEIGHT*sizeof(uint32_t));
    CopyToDeviceInput(&(input[0][0]), device_input, ((INPUT_WIDTH + 128) & 0xFFFFFF80)*INPUT_HEIGHT*sizeof(uint32_t));
    /* End of setup code */

    /* This is the call you will use to time your parallel implementation */
    TIME_IT("opt_2dhisto",
            1100,
            opt_2dhisto(device_input, INPUT_HEIGHT, ((INPUT_WIDTH + 128) & 0xFFFFFF80), Hd);)

    /* Include your teardown code below (temporary variables, function calls, etc.) */
 
    CopyFromDeviceHist(H, Hd);

    FreeDeviceHist(&Hd);
    FreeDeviceInput(device_input);
