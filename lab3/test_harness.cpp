...
int main(int argc, char* argv[])
{
	...
	uint32_t *device_input = (uint32_t *)allocateDeviceMemory(PADDED_WIDTH*INPUT_HEIGHT*sizeof(uint32_t));
	uint32_t *device_bins = (uint32_t *)allocateDeviceMemory(HISTO_HEIGHT * HISTO_WIDTH * sizeof(uint32_t));
	uint8_t *device_bins_byte = (uint8_t *)allocateDeviceMemory(HISTO_HEIGHT * HISTO_WIDTH * sizeof(uint8_t));

	copyToDeviceMemory(device_input, &(input[0][0]), PADDED_WIDTH*INPUT_HEIGHT*sizeof(uint32_t));
	/* End of setup code */

    /* This is the call you will use to time your parallel implementation */
	TIME_IT("opt_2dhisto",
            1000,
            opt_2dhisto(device_input, INPUT_HEIGHT, INPUT_HEIGHT, device_bins, device_bins_byte);)

    /* Include your teardown code below (temporary variables, function calls, etc.)*/
    copyToHostMemory(kernel_bins, device_bins_byte,  HISTO_WIDTH*HISTO_HEIGHT*sizeof(uint8_t));

	freeDeviceMemory(device_bins);
	freeDeviceMemory(device_bins_byte);
