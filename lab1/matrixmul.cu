...
void MatrixMulOnDevice(const Matrix M, const Matrix N, Matrix P)
{
	...
	Matrix Md = AllocateMatrix(M.width, M.height, 0);
	Matrix Nd = AllocateMatrix(N.width, N.height, 0);
	Matrix Pd = AllocateMatrix(P.width, P.height, 0);

	Md = AllocateDeviceMatrix(Md);
	CopyToDeviceMatrix(Md, M);
	Nd = AllocateDeviceMatrix(Nd);
	CopyToDeviceMatrix(Nd, N);
	Pd = AllocateDeviceMatrix(Pd);
